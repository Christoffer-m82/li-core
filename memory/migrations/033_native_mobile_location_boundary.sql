...
CREATE TABLE li_runtime_data.mobile_location_updates(
 update_id UUID PRIMARY KEY, installation_id UUID NOT NULL REFERENCES li_runtime_data.mobile_location_installations(installation_id),
 ...
 UNIQUE(installation_id,update_id)
);
CREATE TABLE li_runtime_data.mobile_location_visit_events(
 event_id UUID PRIMARY KEY, update_id UUID NOT NULL REFERENCES li_runtime_data.mobile_location_updates(update_id),
...
SET search_path=li_runtime_data,li_memory,pg_catalog,pg_temp AS $$ DECLARE v_user UUID; v_applied BOOLEAN:=FALSE; v_source TEXT; v_updated TIMESTAMPTZ; BEGIN
 SELECT id INTO v_user FROM li_memory.users WHERE user_key='christoffer' AND status='active' LIMIT 1;
 IF EXISTS(SELECT 1 FROM li_runtime_data.mobile_location_updates WHERE update_id=p_update AND installation_id=p_installation) THEN RETURN jsonb_build_object('status','idempotent','settings',li_api.get_place_settings()); END IF;
 IF p_country !~ '^[A-Z]{2}$' OR p_permission<>'granted' OR p_permission_checked<p_observed-INTERVAL '24 hours' OR p_observed<NOW()-INTERVAL '24 hours' OR p_observed>NOW()+INTERVAL '5 minutes' THEN RAISE EXCEPTION 'Invalid, stale, or unpermitted coarse observation'; END IF;
 ...
 IF p_event IS NOT NULL THEN
  IF p_classification NOT IN ('overnight','transit') OR p_first IS NULL OR p_last IS NULL OR p_last<p_first OR (p_classification='overnight' AND p_last::date<=p_first::date) THEN RAISE EXCEPTION 'Invalid visit event'; END IF;
...
SET search_path=li_runtime_data,li_memory,pg_catalog,pg_temp AS $$ DECLARE v_user UUID; v_event li_runtime_data.mobile_location_visit_events%ROWTYPE; v_replay RECORD; BEGIN
 SELECT id INTO v_user FROM li_memory.users WHERE user_key='christoffer' AND status='active' LIMIT 1;
 IF p_classification NOT IN ('overnight','transit') OR NOT EXISTS(SELECT 1 FROM li_runtime_data.mobile_location_installations WHERE installation_id=p_installation AND owner_user_id=v_user) THEN RAISE EXCEPTION 'Invalid correction'; END IF;
