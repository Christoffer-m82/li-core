...
def build_capability_inventory(
    *,
    ...
    capabilities = (
        ...
        Capability(id="artifacts", category="Files & artifacts", name="Private artifacts and uploads", purpose="Temporarily analyses uploads and privately stores explicitly saved or Li-generated files.", operations=("temporary analyse", "save", "download", "keep", "delete", "expiry cleanup"), access=("read", "write"), approval="Uploads persist only when explicitly saved; retention changes are user-directed.", persisted=True, retention="Uploads are temporary unless saved; generated files expire on the configured policy unless kept.", web_exposure="available", sensitivity="restricted", boundary="li", status_source="configuration", status=_status(artifact_storage_configured, artifact_storage_configured), status_detail="Private object storage is configured." if artifact_storage_configured else "Private object storage is not configured.", covered_routes=("POST /artifacts/uploads", "POST /artifacts/generated", "GET /artifacts/{artifact_id}", "POST /artifacts/{artifact_id}/retention", "POST /internal/retention/cleanup", "GET /privacy/settings", "POST /privacy/settings")),
