...
def build_capability_inventory(
    *,
    ...
    capabilities = (
        ...
        Capability(id="place-settings", category="Memory & history", name="Private current place", purpose="Stores a manually selected current country and optional town, plus minimal overnight visit events for user-controlled Most visited ordering.", operations=("read current place", "set current place", "pin or suppress country", "confirm overnight visit"), access=("read", "write"), approval="Web updates are explicit user actions; future device updates require OS location permission.", persisted=True, retention="Stores country, optional town, provider permission state, and minimal trip date boundaries only; no precise coordinates or continuous trail.", web_exposure="available", web_path="/settings", sensitivity="restricted", boundary="li", status_source="runtime", status=database_status, status_detail=database_detail, covered_routes=("GET /settings/place", "POST /settings/place", "POST /settings/place/most-visited", "POST /settings/place/visits")),
