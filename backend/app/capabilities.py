...
def build_capability_inventory(
    *,
    ...
    capabilities = (
        ...
        Capability(id="research", category="Research & web", name="Live web research", purpose="Retrieves current public information through the configured research provider and enforces versioned per-specialist freshness rules.", operations=("search", "validate evidence", "read policy"), access=("read",), approval="No approval for research; consequential use remains governed separately.", persisted=False, retention="Provider results are not a separate permanent knowledge store; compact verification metadata may remain in specialist history.", web_exposure="partial", web_path="/backend", boundary="li", status_source="configuration", status=_status(research_available, research_available), status_detail="Research provider is configured." if research_available else "No research provider is configured.", covered_routes=("GET /specialists/freshness-evidence",)),
        ...
        Capability(id="rhythms-open-loops", category="Productivity", name="Rhythms and open loops", purpose="Turns explicit commitments and recurring reviews into privacy-minimised durable work without claiming unconfigured background execution.", operations=("read rhythms", "list", "create approved sensitive commitment", "postpone", "raise", "close"), access=("read", "write with approval"), approval="Rhythms are preview-only; sensitive commitments and all external provider mutations retain approval.", persisted=True, retention="Summaries and lifecycle metadata persist until closure under task retention; raw messages are not duplicated.", web_exposure="available", web_path="/backend", sensitivity="personal", boundary="li", status_source="runtime", status=database_status, status_detail="Morning, Friday, monthly, quarterly, and annual definitions are preview-only.", covered_routes=("GET /rhythms", "GET /open-loops", "POST /li/open-loops", "POST /li/open-loops/{open_loop_id}/{transition}")),
