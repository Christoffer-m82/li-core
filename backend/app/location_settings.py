...
def minimal_location_context(message: str, place: CurrentPlace | None) -> str | None:
    if not place or not place.country_code or not location_is_relevant(message):
        return None
    town = f"; town/city: {place.town_city}" if place.town_city else ""
    prompt = " Ask for town/city only if materially needed." if not place.town_city and town_is_useful(message) else ""
    return (
...
def qualifying_visit_count(events: list[VisitEvent], *, today: date | None = None) -> dict[str, int]:
    ...
    trips: dict[str, set[tuple[date, date]]] = {}
    for event in events:
        if event.overnight_confirmed and event.last_seen >= cutoff:
            trips.setdefault(event.country_code, set()).add((event.first_seen, event.last_seen))
    return {country: len(values) for country, values in trips.items()}
...
def promoted_countries(events: list[VisitEvent], preferences: list[MostVisitedPreference]) -> list[str]:
    ...
    existing = [item.country_code for item in preferences if item.state == "automatic" and item.country_code not in automatic]
    return list(dict.fromkeys([*pinned, *automatic, *existing]))
