...
def location_is_relevant(message: str) -> bool:
    return bool(_LOCATION_RELEVANT.search(message)) or any(has_term(message, term) for term in (
        "near me", "nearby", "local", "weather", "restaurant", "shop", "service", "law",
        "legal", "tax", "health", "doctor", "insurance", "visa", "travel", "trip", "hotel",
        "flight", "garden", "plant", "delivery", "currency", "jurisdiction", "license",
        "permit", "government", "bank",
    ))


def town_is_useful(message: str) -> bool:
    return bool(_TOWN_RELEVANT.search(message)) or any(has_term(message, term) for term in (
        "near me", "nearby", "restaurant", "cafe", "shop", "local service", "weather",
        "doctor", "hospital", "hotel", "airport", "delivery", "garden center",
    ))
