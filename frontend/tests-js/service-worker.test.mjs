...
function worker({ offline = false, ok = true, redirected = false, type = 'basic', writeFails = false, cacheHit = true, cacheOpenFails = false } = {}) {
  ...
  vm.runInNewContext(source, {
    ...
    caches: {
      async open(name) { assert.equal(name, 'li-shell-v19'); if (cacheOpenFails) throw new Error('cache unavailable'); return cache; },
      async keys() { return ['li-shell-v18', 'li-shell-v19', 'other-app']; },
...
test('installation caches static assets only and activation preserves unrelated caches', async () => {
  ...
  assert.deepEqual(app.deleted, ['li-shell-v18']);
});
