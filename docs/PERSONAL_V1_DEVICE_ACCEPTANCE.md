...
| Open My Finances, switch between Avanza and Crypto, and use one clearly synthetic holding | Values remain separated by currency; unpriced/stale states are explicit; create/edit/archive affect only the synthetic holding; no broker login, market quote, trade, memory, or specialist sharing occurs | NOT RUN — source and schema 0.42 are deployed. The separately action-time-confirmed `LIOS42` archive passed through the governed backend capability with every non-target active Avanza holding unchanged; the physical-device owner journey remains open |
