...
| OM-010 | Release the private Finance and Calendar workspaces | The [accepted architecture](../system/FINANCE_CALENDAR_ARCHITECTURE.md) defines a governed Calendar read UI and schema-0.42 owner-entered Avanza/Crypto portfolio without brokerage credentials or trading authority. The [workspace release](releases/2026-09-07-a7601b7-staging.md) records the schema/UI/Finance rollout; the [diagnostic backend release](releases/2026-09-08-db1d17c-staging.md) records one safely classified live OAuth refresh-grant failure. | Complete a separately authorized owner consent and refresh-token replacement, deploy its reviewed numeric secret reference, and pass a newly authorized bounded Calendar read; finish the exact synthetic Finance archive check; record owner acceptance on Android phone/tablet and installed Windows PWA plus stability observation. Automatic market quotes remain a later reviewed extension. |
