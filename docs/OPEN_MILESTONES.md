...
| OM-008 | Extend repository CI and add database integration validation | The tracked docs workflow validates Markdown links and anchors. No tracked workflow runs the Python/native component checks, secret scanning, or representative database migrations against a disposable database. | Tracked CI running isolated component checks, docs-link validation, secret scanning, and representative migration behavior tests. |
