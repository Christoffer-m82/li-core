...
| OM-010 | Release the private Finance and Calendar workspaces | The [accepted architecture](../system/FINANCE_CALENDAR_ARCHITECTURE.md) defines a governed Calendar read UI and schema-0.42 owner-entered Avanza/Crypto portfolio without brokerage credentials or trading authority. Repository implementation does not prove applied schema, deployment, live data quality, or device behavior. The [release-readiness record](releases/2026-09-07-f8642e5-staging-readiness.md) fixes the dependency order, current rollback baselines and stop conditions without authorizing external work. | Full schema-0.42 migration replay and security denials, exact authorized backup/rehearsal/migration, coordinated backend/web candidates, provider-backed Calendar read, and owner acceptance on Android phone/tablet and installed Windows PWA. Automatic market quotes remain a later reviewed extension. |
