...
| OM-008 | Add database and native integration validation to repository CI | The tracked workflows validate Markdown links and anchors, run the backend, frontend, and native-gateway Python checks in isolation, and reject tracked secret files and high-confidence credential patterns. No tracked workflow runs the native platform checks or representative database migrations against a disposable database. | Tracked CI running native platform checks and representative migration behavior tests against a disposable database, while preserving the existing component, documentation, and secret checks. |
