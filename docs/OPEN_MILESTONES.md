...
| OM-008 | Add native integration validation to repository CI | The tracked workflows validate Markdown links and anchors, run the backend, frontend, and native-gateway Python checks in isolation, reject tracked secret files and high-confidence credential patterns, and rehearse the explicit migration history against disposable PostgreSQL with representative security assertions. No tracked workflow runs the native platform checks. | Tracked CI running the iOS and Android platform checks while preserving the existing component, documentation, secret, and disposable-database checks. |
