...
| Owner profile photos use an isolated private cross-device service, not model-facing artifacts | [Owner Profile Photo ADR](../system/OWNER_PROFILE_PHOTO_ARCHITECTURE.md) | Proposed design with local domain tests only; hosted activation cost-blocked; no deployed photo feature |
