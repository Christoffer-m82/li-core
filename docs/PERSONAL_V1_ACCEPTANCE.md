...
| Finance and calendar workspaces | Avanza/Crypto positions remain private and non-transactional; per-currency values and stale/unpriced states are honest; Calendar is Monday-first with weekend distinction, all-day safety and bounded governed reads | Source implementation is locally verified: the full disposable migration manifest passed through schema 0.42 with runtime allow/direct-role denials; backend, frontend and browser suites passed; and static responsive/HTML checks passed. The [staging-readiness record](releases/2026-09-07-f8642e5-staging-readiness.md) prepares the protected sequence but does not authorize or prove it. Coordinated migration/deployment, provider-backed Calendar display, and Android phone/tablet plus installed-Windows owner acceptance require separate evidence; automatic market quotes are not part of the foundation |
